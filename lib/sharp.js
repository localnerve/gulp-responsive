...
export default async function sharpVinyl (file, config, options) {
  const packageJsonData = await parsePackageJson()
  const PLUGIN_NAME = packageJsonData.name
  ...
  try {
    ...
    if (width || height) {
      if (
        ...
        if (options.errorOnEnlargement) {
          ...
          if (!options.silent) {
            flog(PLUGIN_NAME + ': (skip for processing)', styleText('red', message))
          }
          // passing a null file to the callback stops a new image being added to the pipeline for this config
          return null
        }
        if (!options.silent) {
          flog(PLUGIN_NAME + ': (skip for enlargement)', styleText('yellow', message))
        }
      }
    }
    ...
    const allOps = toFormat.map(async format => {
      ...
      if (!options.silent) {
        flog(
          PLUGIN_NAME + ':',
          styleText('green', file.relative + ' -> ' + newFile.relative)
        )
      }

      if (typeof options.postprocess === 'function') {
        options.postprocess(file, config, newFile)
      }
